...
function Face_Node(_face_number, _move = 0, _adjacency_array = [], _cooldown = 0) constructor
{
...
function get_hovered_adjacent_move_names(_face_number) {
	...
	for (var i = 0; i < 3; i++) {
		...
		_display_string = string_concat(_display_string, global.die_graph_array[_adjacent_nodes[i] - 1].move);
		if (i+1 != 3) {
			_display_string = string_concat(_display_string, ",\n");
		}
	}
